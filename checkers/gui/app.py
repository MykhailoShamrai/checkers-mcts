...
def _status_text(state: GameState, config: GameConfig, game_over: bool) -> str:
    if game_over:
        winner = state.winner()
        if winner is None:
            return f"DRAW (move limit {state.move_limit} reached) — press R to restart"
        name = "WHITE" if winner == C.WHITE else "BLACK"
        return f"{name} wins! — press R to restart"
    to_move = "WHITE" if state.to_move == C.WHITE else "BLACK"
    kind = _player_name(state, config)
    return f"{to_move}'s turn ({kind}) | Ply {state.ply}/{state.move_limit}"
